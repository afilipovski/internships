...
function SupervisorCalendar(props) {
    ...
    const supervisorApprove = async () => {
        try {
            await InternshipSupervisorRepository.approveInternship(user.id,internshipId);
    ...
    const revokeApproval = async () => {
        try {
            await InternshipSupervisorRepository.revokeApprovalInternship(user.id,internshipId);
            // Reload internship to reflect the new week
            loadInternship();
        } catch (error) {
            console.error('Failed to add internship week', error);
    ...
    return (
        <div className="md:h-[93vh] w-full flex flex-col lg:flex-row">
            <div className="md:basis-1/3 lg:basis-1/4 flex flex-col justify-start px-4 py-4 shadowCard h-max">
                ...
                <div className="flex justify-center flex-row">
                    {internship && internship.status==="PENDING_COMPANY_REVIEW" && <button className="bg-finkiO text-white p-3 rounded-xl mx-2" onClick={supervisorApprove}>
                        Approve
                    </button>}
...
SupervisorCalendar.defaultProps = {};
